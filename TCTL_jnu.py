...
import ReadData_JNU_5000 as RD
import torch
...
def train(model, optimizer, epoch, data_src, data_tar, sample_len):
    ...
    res_e = 'Epoch: [{}/{}], training loss: {:.6f}, correct: [{}/{}], training accuracy: {:.4f}%'.format(epoch, N_EPOCH, total_loss_train, correct, len(data_src.dataset), acc)
    # tqdm.write(res_e)
...
def test(model, data_tar, e, sample_len):
    ...
    with torch.no_grad():
        ...
        res = 'Test: total loss: {:.6f}, correct: [{}/{}], testing accuracy: {:.4f}%'.format(total_loss_test, correct, len(data_tar.dataset), accuracy)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--src', type=int, choices=[600, 800, 1000], default=600)
    parser.add_argument('--tar', type=int, choices=[600, 800, 1000], default=800)
    args = parser.parse_args()

    path_source = f'data_jnu/{args.src}'
    path_target = f'data_jnu/{args.tar}'
    ...
    RESULT_TEST = []
    N_EPOCH = 100  # 900
    x_train_source, y_train_source, x_validate, y_validate, x_test, y_test = RD.get_data(path_source, rate, stride, sample_len)
    x_train_target, y_train_target, x_validate, y_validate, x_test, y_test = RD.get_data(path_target, rate, stride, sample_len)

    # 增加一个维度以满足conv1D的输入要求[]
    x_train_source, x_train_target = x_train_source[:, np.newaxis, :], x_train_target[:, np.newaxis, :]  # 后面训练模块增加通道数
    ...
    for e in range(1, N_EPOCH):
        ...
        # train_dataloader_target = DataLoader(train_data_target, batch_size=32, shuffle=True, drop_last=True)
        # print("epoch=%d", e)
        model = train(model=model, optimizer=optimizer, epoch=e, data_src=train_dataloader_source, data_tar=train_dataloader_target, sample_len=sample_len)
        res = test(model, train_dataloader_target, e, sample_len=sample_len)
        print(f'Epoch: {e}, {res}')
    print(RESULT_TEST[-1])
    # print(RESULT_TRAIN)
